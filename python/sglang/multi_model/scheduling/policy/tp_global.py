...
from sglang.multi_model.scheduling.policy.tracker import RequestViolationTracker, ModelRequestTracker
...
class TPGlobalPolicy(GlobalPolicy):
    def __init__(
        ...
        self.num_rank0_gpus = num_gpus # 作为rank0的GPU数
        ...
        self.request_tracker = ModelRequestTracker(window_size_seconds=30)
    ...
    def _check_idle_instance_eviction(
        ...
        """
        evictable_instances: Dict[int, List[ModelInstanceState]] = {} # {gpu_id, [ModelInstanceState]}
        if self.first_request_time is None:
            for model_name, queue in model_queues.items():
                if queue.get_last_arrival_time() != float("-inf"):
                    self.first_request_time = queue.get_last_arrival_time() # 首批请求到达后，全局初始化活跃时间
                    ...
                    break
        if self.first_request_time is None: return evictable_instances # 无到达请求
        idle_models = set()
        current_time = time.time()
        for model_name, queue in model_queues.items():
            # 更新活跃时间
            ...
            if last_request_time is not None:
                ...
                has_requests = queue.get_num_unfinished_reqs() > 0
                if model_name in [name for models in gpu_to_model_mapping.values() for name in models]:
                ...
                # 长期空闲且无未完成请求
                if idle_time > self.MODEL_IDLE_THRESHOLD and not has_requests: idle_models.add(model_name)
        # 筛选可驱逐实例
        for gpu_id, active_instances in gpu_to_instance_mapping.items():
            if not active_instances: continue
            evictable_instances[gpu_id] = []
            for instance in active_instances:
                if instance.model_name in idle_models:
                    evictable_instances[gpu_id].append(instance)        
    ...
    def _find_optimal_migrations_by_violation(
        self,
        single_model_instance_state_dict: Dict[str, List[ModelInstanceState]],
        model_queues: Dict[str, ModelQueueTracker],
        ...
        根据违规结果确定迁移方案
            ...
            返回迁移方案[(model_name, instance_idx, origin_gpu_id, target_gpu_id)]
        """
        gpu_violation_stats = self._calculate_gpu_violation_stats(gpu_to_model_mapping, model_violation_stats)
        # 逐GPU、模型统计违规数据
        ...
        # 初始化{gpu_id: (model_name, violation_proportion, avg_budget)}，只被rank0 GPU统计一次
        ...
        for gpu_id in self.all_gpus:
            ...
            for model_name in gpu_to_model_mapping.get(gpu_id, []):
                if model_name in model_violation_stats:
                    ...
                    gpu_models_with_violations[gpu_id].append((model_name, model_violation_stats[model_name][1], avg_budget))
            # 按违规率升序，时间预算降序排列
            gpu_models_with_violations[gpu_id].sort(key=lambda x: (x[1], -x[2]))
        # 遍历查找迁移方案
        for high_idx in range(len(sorted_gpus)): # 违规率最高
            high_gpu_id, high_violation = sorted_gpus[high_idx]
            if not gpu_models_with_violations[high_gpu_id]: continue
            for low_idx in range(len(sorted_gpus) - 1, high_idx, -1): # 违规率最低
                low_gpu_id, low_violation = sorted_gpus[low_idx]
                if high_violation - low_violation < self.VIOLATION_PROPORTION_THRESHOLD: break
                ...
                for model_name, violation, avg_budget in gpu_models_with_violations[high_gpu_id]:
                    queue = model_queues.get(model_name)
                    if queue:
                        model_reqs = queue.get_num_unfinished_reqs()
                ...
                for model_name, violation, avg_budget, model_reqs in sorted_models:
                    instance_idx = None
                    for instance in single_model_instance_state_dict.get(model_name):
                    ...
                    if gpu_available_memory[low_gpu_id] < model_memory: continue
                    # 模拟迁移结果
                    ...
                    # 模拟迁移后违规情况
                    simulated_gpu_violation_stats = self._calculate_gpu_violation_stats(simulated_mapping, model_violation_stats)
                    logger.info(f"Migration Placement: model {model_name} (with {model_reqs} requests) from {high_gpu_id} to {low_gpu_id}")
                    for gpu_id, stats in simulated_gpu_violation_stats.items():
                        logger.info(f"  Simulated GPU {gpu_id}, violation_proportion: {stats[1]}, total_reqs: {stats[2]}")
                    # 迁移后违规率平衡则返回结果
                    if self._is_placement_stable_by_violation(simulated_gpu_violation_stats):
                        return [(model_name, instance_idx, high_gpu_id, low_gpu_id)]
    ...
    def _calculate_memory_per_request(
        ...
        self.request_tracker.update_request_history(model_queues)
        gpu_memory_per_request = {}
        for gpu_id, model_names in gpu_to_model_mapping.items():
            total_model_memory, total_reqs = 0, 0
            for model_name in model_names:
                total_model_memory += self.model_weights_info[model_name]["model_size"]
                # 平滑均值（时间窗口内均值）
                total_reqs += self.request_tracker.get_model_request_stats(model_name)
            memory_available_for_requests = self.gpu_mem - total_model_memory
            memory_per_request = memory_available_for_requests / (total_reqs if total_reqs > 0 else 1)
        ...
        for i in range(len(gpu_ids)):
            for j in range(i+1, len(gpu_ids)):
                ...
                if reqs_i == 0 and reqs_j == 0:
                    unstable_pairs += 1
                    continue
                if reqs_i == 0: mem_i = float('inf')
                if reqs_j == 0: mem_j = float('inf')
    ...
    def _find_optimal_migrations_by_memory(
        ...
        根据显存分布确定迁移方案
            ...
            返回迁移方案[(model_name, instance_idx, origin_gpu_id, target_gpu_id)]
        """
        memory_per_request = self._calculate_memory_per_request(gpu_to_model_mapping, model_queues)
        # 逐GPU、模型统计显存数据
        for gpu_id, (memory_per_req, total_reqs) in memory_per_request.items():
            ...
            for model_name in gpu_to_model_mapping[gpu_id]:
                model_reqs = self.request_tracker.get_model_request_stats(model_name)
                model_memory = self.model_weights_info[model_name]["model_size"]
                logger.info(f"    Model {model_name}:")
                logger.info(f"      Requests (smoothed): {model_reqs:.2f}")
                logger.info(f"      Model memory: {model_memory:.2f} GB")
        # 显存不均衡时迁移
        current_unstable_pairs = self._count_unstable_pairs(memory_per_request)
        ...
        # 按请求平均显存升序排列GPU
        sorted_gpus = sorted(
            [(gpu_id, memory, reqs) for gpu_id, (memory, reqs) in memory_per_request.items()],
            key=lambda x: x[1]
        )
        # 获取激活实例数量
        gpu_to_single_active_instances = self._get_gpu_to_instances_mapping(single_model_instance_state_dict)
        gpu_active_instance_count = {gpu_id: len(instances) for gpu_id, instances in gpu_to_single_active_instances.items()}
        # 遍历查找迁移方案
        for low_idx in range(len(sorted_gpus)): # 显存最紧张
            low_gpu_id, low_memory, _ = sorted_gpus[low_idx]
            if gpu_active_instance_count[low_gpu_id] == 1: continue
            for high_idx in range(len(sorted_gpus) - 1, low_idx, -1): # 显存最宽松
                high_gpu_id, high_memory, _ = sorted_gpus[high_idx]
                if high_memory / low_memory <= self.MEMORY_PER_REQUEST_RATIO_THRESHOLD: continue
                # 按未完成请求数升序排列，优先迁移平均请求少的模型
                models_with_req_count = []
                ...
                for model_name, model_reqs in sorted_models:
                    ...
                    # 目标GPU显存不足则放弃迁移
                    model_memory = self.model_weights_info[model_name]["model_size"]
                    if gpu_available_memory[high_gpu_id] < model_memory: continue
                    # 模拟迁移结果
                    simulated_mapping = {gpu_id: models.copy() for gpu_id, models in gpu_to_model_mapping.items()}
                    simulated_mapping[low_gpu_id].remove(model_name)
                    simulated_mapping[high_gpu_id].append(model_name)
                    # 模拟迁移后显存情况
                    simulated_memory_per_request = self._calculate_memory_per_request(simulated_mapping, model_queues)
                    ...
                    if simulated_unstable_pairs < current_unstable_pairs:
                        return [(model_name, instance_idx, low_gpu_id, high_gpu_id)]
    ...
    def _find_optimal_migrations(
        ...
        根据全局迁移策略查找迁移方案
            ...
            返回迁移方案[(model_name, instance_idx, origin_gpu_id, target_gpu_id)]
        """
        single_model_instance_state_dict = {
            model_name: [instance for instance in instances if len(instance.gpu_ids) == 1]
            for model_name, instances in model_instance_state_dict.items()
        }
    ...
    def gen_actions(
        ...
        idle_instances: Dict[int, List[ModelInstanceState]] = self._check_idle_instance_eviction(model_queues, gpu_to_model_mapping, gpu_to_active_instances)
        for gpu_id, evictable_instances in idle_instances.items():
            for instance in evictable_instances:
                instance_key = (instance.model_name, instance.instance_idx)
                ...
                model_instance_to_action_dict.setdefault(instance_key, []).append(deactivate_action)
                logger.info(f"ACTION: deactivate {instance.model_name}: {instance.instance_idx} on GPU {gpu_id}. Reason: idle instance eviction")
        ...
        valid_migrations = []
        for model_name, instance_idx, origin_gpu_id, target_gpu_id in migrations:
            instances = gpu_to_active_instances[target_gpu_id]
            if self.enable_worker_pool and len(instances) >= self.workers_per_gpu: # 超出worker pool容量
                logger.info(f"Skipping migration instance {instance_idx} of {model_name} to {target_gpu_id} because target GPU {target_gpu_id} already has {len(instances)} active instances")
                continue
            gpu_available_memory[target_gpu_id] -= self.model_weights_info[model_name]["model_size"]
            valid_migrations.append((model_name, instance_idx, origin_gpu_id, target_gpu_id))
            logger.info(f"PLANNING: migrate instance {instance_idx} of {model_name} from {origin_gpu_id} to GPU {target_gpu_id}")
        ...
        for model_name, instance_idx, origin_gpu_id, target_gpu_id in valid_migrations:
            # 清理原模型
            ...
            if source_instance_key not in idle_instance_keys:
                model_instance_to_action_dict.setdefault(source_instance_key, []).append(
                    DeactivateAction(
                        ...
                        preempt_mode="RECOMPUTE",
                        evict_waiting_requests=True,
                        gpu_id=origin_gpu_id
                    )
                )
                # 在目标GPU激活模型
                new_instance_idx = target_gpu_id if self.enable_worker_pool else instance_idx # worker pool时instance编号由GPU决定
                target_instance_key = (model_name, new_instance_idx)
                model_instance_to_action_dict.setdefault(target_instance_key, []).append(
                    ActivateAction(
                        model_name=model_name,
                        instance_idx=new_instance_idx,
                        memory_pool_size=self.MEMORY_POOL_BUDGET,
                        gpu_id=target_gpu_id,
                    )
                )
                logger.info(f"ACTION: deactivate {model_name}: {instance_idx} on GPU {origin_gpu_id} and activate {model_name} on GPU {target_gpu_id}. Reason: migrate model")
