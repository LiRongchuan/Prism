        ...
        if zmq_sockets:
            for name, socket in zmq_sockets.items():
                try:
                    ...
                    logger.warning(f"Failed to close {name} socket: {e}")

        # Remove IPC files
        if ipc_files:
            for ipc_path in ipc_files:
                if isinstance(ipc_path, str) and ipc_path.startswith("ipc://"):
                    file_path = ipc_path[6:]  # Remove the "ipc://" prefix
                    try:
                        if os.path.exists(file_path):
                            os.unlink(file_path)
                            logger.info(f"Removed IPC file: {file_path}")
                    except Exception as e:
                        logger.warning(f"Failed to remove IPC file {file_path}: {e}")

        logger.info(f"{component_name}{location_info} cleanup completed")
        return True
    except Exception as e:
